...
def on_message(client: mqtt.Client, userdata, msg: mqtt.MQTTMessage):
    ...
    if DeviceType.UPS.name in topic_levels:
        if settings.settings["ups_module_enabled"] == False:
            logger.warning("Arrived UPS telemetry, but the module is not active")
            return
        else:
...
def start_consume():
    ...
    _mqttc.connect(settings.settings['mqtt_broker_host'], 1883, 60)
    _mqttc.loop_start()
